...
object Expression {
  private def createDefaultVariables = {
    val vars = new mutable.HashMap[String, Double]
    vars.put("pi", Math.PI)
    vars.put("π", Math.PI)
...
class Expression {
  ...
  final private var variables: mutable.Map[String, Double] = null
  ...
  def this(existing: Expression) {
    this()
    this.tokens = Array.ofDim(existing.tokens.length)
    existing.tokens.copyToArray(this.tokens)

    this.variables = new mutable.HashMap[String, Double]()
    this.variables ++= existing.variables
    this.userFunctionNames = existing.userFunctionNames
  }

  def this(tokens: Array[Token], userFunctionNames: Set[String] = Set()) {
    this()
    this.tokens = tokens
  ...
  private def checkVariableName(name: String): Unit = if (this.userFunctionNames.contains(name) || Functions.getBuiltinFunction(name) != null) throw new IllegalArgumentException("The variable name '" + name + "' is invalid. Since there exists a function with the same name")

  def setVariables(variables: Map[String, Double]): Expression = {
    for ((k, v) <- variables) {
  ...
  def getVariableNames: Set[String] = {
    val variables = new mutable.HashSet[String]
    for (t <- tokens) {
  ...
  def validate(checkVariablesSet: Boolean): ValidationResult = {
    val errors = new ArrayBuffer[String](0)
    if (checkVariablesSet) /* check that all vars have a value set */ for (t <- this.tokens) {
      if (t.getType == Token.TOKEN_VARIABLE) {
        ...
        if (!variables.contains(`var`)) errors += ("The setVariable '" + `var` + "' has not been set")
      }
    }
    ...
    for (tok <- this.tokens) {
      tok.getType match {
        ...
        case Token.TOKEN_FUNCTION =>
          ...
          if (argsNum > count) errors += ("Not enough arguments for '" + func.getName + "'")
    ...
    if (count > 1) errors += "Too many operands"
    if (errors.isEmpty) ValidationResult.SUCCESS
    else new ValidationResult(false, errors.toList)
  }
  ...
  def evaluate: Double = {
    ...
    for (t <- tokens) {
      if (t.getType == Token.TOKEN_NUMBER) output.push(t.asInstanceOf[NumberToken].getValue)
        ...
        val value = this.variables.getOrElse(name, throw new IllegalArgumentException("No value has been set for the setVariable '" + name + "'."))
        output.push(value)
      }
