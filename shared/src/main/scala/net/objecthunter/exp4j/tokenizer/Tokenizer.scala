...
object Tokenizer {
  private def isNumeric(ch: Char, lastCharE: Boolean) = ch.isDigit || ch == '.' || ch == 'e' || ch == 'E' || (lastCharE && (ch == '-' || ch == '+'))

  def isAlphabetic(codePoint: Char): Boolean = codePoint.isLetter

  def isVariableOrFunctionCharacter(codePoint: Char): Boolean = isAlphabetic(codePoint) || codePoint.isDigit || codePoint == '_' || codePoint == '.'
}

class Tokenizer {
  final private var expression: Array[Char] = null
  final private var expressionLength: Int = 0
  final private var userFunctions: Map[String, function.Function] = Map()
  final private var userOperators: Map[String, Operator] = Map()
  final private var variableNames: Set[String] = Set()
  final private var implicitMultiplication = false
  private var pos = 0
  private var lastToken: Token = null

  def this(expression: String, userFunctions: Map[String, function.Function], userOperators: Map[String, Operator], variableNames: Set[String], implicitMultiplication: Boolean) {
    this()
  ...
  def this(expression: String, userFunctions: Map[String, function.Function], userOperators: Map[String, Operator], variableNames: Set[String]) {
    this()
    this.expression = expression.trim.toCharArray
    this.expressionLength = this.expression.length
    this.userFunctions = userFunctions
  ...
  private def getFunction(name: String) = {
    ...
    if (this.userFunctions != null) f = this.userFunctions.getOrElse(name, Functions.getBuiltinFunction(name))
    f
  }
  ...
  private def getOperator(symbol: String) = {
    ...
    if (this.userOperators != null) op = this.userOperators.getOrElse(symbol, null)
