...
class TokenizerUnknownTokenOrVariableTest {
  ...
  def testTokenizationOfUnknownVariable(): Unit = {
    val tokenizer = new Tokenizer("3 + x", Map(), Map(), Set())
    while ( {
      tokenizer.hasNext
    }) tokenizer.nextToken
  }
  ...
  def testTokenizationOfUnknownVariable2Details(): Unit = {
    val tokenizer = new Tokenizer("x + 3", Map(), Map(), Set())
    try {
      tokenizer.nextToken
      Assert.fail("Variable 'x' should be unknown!")
    } catch {
  ...
  def testTokenizationOfUnknownFunction(): Unit = {
    val tokenizer = new Tokenizer("3 + p(1)", Map(), Map(), Set())
    while ( {
      tokenizer.hasNext
    }) tokenizer.nextToken
  }
  ...
  def testTokenizationOfUnknownFunction2Details(): Unit = {
    val tokenizer = new Tokenizer("p(1) + 3", Map(), Map(), Set())
    try {
      tokenizer.nextToken
      Assert.fail("Function 'p' should be unknown!")
    } catch {
