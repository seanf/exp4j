...
class TokenizerUnknownTokenOrVariableTest {
  @Test(expected = classOf[Nothing])
  @throws[Exception]
  def testTokenizationOfUnknownVariable(): Unit = {
    val tokenizer = new Nothing("3 + x", null, null, null)
    while ( {
      tokenizer.hasNext
    }) tokenizer.nextToken
  }
  ...
  def testTokenizationOfUnknownVariable2Details(): Unit = {
    val tokenizer = new Nothing("x + 3", null, null, null)
    try {
      tokenizer.nextToken
      Assert.fail("Variable 'x' should be unknown!")
    } catch {
  ...
  def testTokenizationOfUnknownFunction(): Unit = {
    val tokenizer = new Nothing("3 + p(1)", null, null, null)
    while ( {
      tokenizer.hasNext
    }) tokenizer.nextToken
  }
  ...
  def testTokenizationOfUnknownFunction2Details(): Unit = {
    val tokenizer = new Nothing("p(1) + 3", null, null, null)
    try {
      tokenizer.nextToken
      Assert.fail("Function 'p' should be unknown!")
    } catch {
